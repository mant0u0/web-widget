...
const NestedItemManager = () => {
  ...
  const buildTextTree = (
    ...
    itemsList.forEach((item) => {
      ...
      if (showType && item.itemType) {
        result += `${indent}  [ 類型 ] ${item.itemType}\n`;
      }

      // 如果選擇顯示備註，且有備註資訊
      if (showNotes && item.notes) {
        result += `${indent}  [ 備註 ] ${item.notes}\n`;
      }

      // 遞迴處理子項目
      if (item.children.length > 0) {
        result += buildTextTree(item.children, showType, showNotes, depth + 1);
      }
    });
  ...
  const handleTypeCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 使用 setTimeout 確保在狀態更新後生成新的文字
    setTimeout(() => {
      const newText = buildTextTree(items, e.target.checked, showItemNotes);
      setFormattedText(newText);
    }, 0);
  };

  // 處理顯示備註選項變更
  const handleNotesCheckboxChange = (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    setShowItemNotes(e.target.checked);
    // 使用 setTimeout 確保在狀態更新後生成新的文字
    setTimeout(() => {
      const newText = buildTextTree(items, showItemType, e.target.checked);
      setFormattedText(newText);
    }, 0);
  };
          ...
          <DialogContent className="sm:max-w-2xl">
            ...
            <div className="mb-4 flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  id="showType"
                  checked={showItemType}
                  onChange={handleTypeCheckboxChange}
                  className="h-4 w-4 rounded border-gray-300 text-blue-600"
                />
                <label htmlFor="showType" className="text-sm">
                  顯示類型
                </label>
              </div>
            ...
            <DialogFooter>
              <Button
                ...
                onClick={() => {
                  window.open(
                    `https://chatgpt.com/?&q=以下是客戶的網頁規劃結構，請幫我整理成詳細的需求表、規格表、報價單，我需要提供給客戶：${encodeURIComponent(
                      "\n" + formattedText,
                    )}`,
                  );
                }}
